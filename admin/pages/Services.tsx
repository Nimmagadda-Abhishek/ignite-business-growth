...
const Services = () => {
  ...
  const [form, setForm] = useState({ service_id: "", title: "", description: "", features: "", technologies: "", pricing: "", delivery_time: "", image_url: "" });
  ...
  const exportCSV = () => {
    const header = "Service ID,Title,Description,Features,Technologies,Pricing,Delivery Time,Image URL\n";
    const rows = filteredServices.map(s =>
      `"${s.service_id}","${s.title}","${s.description.replace(/"/g, '""')}","${s.features}","${s.technologies}","${s.pricing}","${s.delivery_time}","${s.image_url || ''}"`
    ).join("\n");
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      setEditId(null);
      setForm({ service_id: '', title: '', description: '', features: '', technologies: '', pricing: '', delivery_time: '', image_url: '' });
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 mb-6 flex-wrap">
        ...
        {editId && (
          <button type="button" onClick={() => { setEditId(null); setForm({ service_id: '', title: '', description: '', features: '', technologies: '', pricing: '', delivery_time: '', image_url: '' }); }} className="btn-outline px-6 py-2 rounded-lg">
            Cancel
          </button>
        )}
      </form>
          ...
          {filteredServices.map(s => (
            <li key={s.id} className="flex flex-col md:flex-row md:items-center justify-between bg-gray-50 rounded-lg p-4 shadow hover:shadow-md transition">
              <div>
                ...
                {s.image_url && (
                  <img src={s.image_url} alt="Service" className="w-20 h-20 object-cover rounded-lg border mt-2" />
                )}
              </div>
              <div className="flex gap-2 mt-2 md:mt-0">
                <button onClick={() => { setEditId(s.id); setForm({ service_id: s.service_id, title: s.title, description: s.description, features: s.features, technologies: s.technologies, pricing: s.pricing, delivery_time: s.delivery_time, image_url: s.image_url || '' }); }} className="btn-outline px-4 py-1 rounded-lg">Edit</button>
