...
const resource = "quote_submissions";
...
const QuoteSubmissions = () => {
  ...
  const { data: submissions = [], isLoading } = useQuery<QuoteSubmission[]>({ queryKey: [resource], queryFn: getAll.bind(null, resource) });
  ...
  const filtered = useMemo(() =>
    submissions.filter(s =>
      ...
      s.budget.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const exportCSV = () => {
    const header = "Name,Email,Phone,Company,Service,Budget,Message,Submitted At\n";
    const rows = filtered.map(s =>
      `"${s.name}","${s.email}","${s.phone}","${s.company}","${s.service}","${s.budget}","${s.message.replace(/"/g, '""')}","${s.submitted_at}"`
    ).join("\n");
    ...
    a.href = url;
    a.download = 'quote_submissions.csv';
    a.click();
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <h1 className="text-3xl font-bold mb-4 text-primary">Quote Request Submissions</h1>
          ...
          {filtered.map(s => (
            <li key={s.id} className="flex flex-col md:flex-row md:items-center justify-between bg-gray-50 rounded-lg p-4 shadow hover:shadow-md transition">
              <div>
                ...
                <div className="text-xs text-gray-400">{s.company} | {s.service} | {s.budget}</div>
