...
const resource = "additional_services";
...
const AdditionalServices = () => {
  ...
  const { data: services = [], isLoading } = useQuery<AdditionalService[]>({ queryKey: [resource], queryFn: getAll.bind(null, resource) });
  ...
  const createMutation = useMutation({
    mutationFn: (data: Omit<AdditionalService, 'id'>) => createOne(resource, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [resource] }),
  });
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Omit<AdditionalService, 'id'> }) => updateOne(resource, id, data),
    onSuccess: () => {
      setEditId(null);
  ...
  const exportCSV = () => {
    ...
    const rows = filteredServices.map(s =>
      `"${s.title}","${s.description.replace(/"/g, '""')}"`
    ).join("\n");
    ...
    a.href = url;
    a.download = 'additional_services.csv';
    a.click();
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <h1 className="text-3xl font-bold mb-4 text-primary">Manage Additional Services</h1>
          ...
          {filteredServices.map(s => (
            <li key={s.id} className="flex flex-col md:flex-row md:items-center justify-between bg-gray-50 rounded-lg p-4 shadow hover:shadow-md transition">
              ...
              <div className="flex gap-2 mt-2 md:mt-0">
                <button onClick={() => { setEditId(s.id); setForm({ title: s.title, description: s.description }); }} className="btn-outline px-4 py-1 rounded-lg">Edit</button>
