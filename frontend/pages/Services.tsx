...
const Services = () => {
  ...
  useEffect(() => {
    getAll('services')
      .then(data => {
        // Parse features and technologies as arrays if stored as CSV
        const parsed = data.map((s: any) => ({
          ...s,
          features: typeof s.features === 'string' ? s.features.split(',').map((f: string) => f.trim()) : s.features,
          technologies: typeof s.technologies === 'string' ? s.technologies.split(',').map((t: string) => t.trim()) : s.technologies,
          icon: iconMap[s.service_id] || Code,
        }));
        setServices(parsed);
      });
  }, []);
              ...
              <div key={service.id} className={`flex flex-col lg:flex-row items-center gap-12 ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-6 mb-6">
                    ...
                    <div>
                      <div className="font-semibold">{service.delivery_time}</div>
                ...
                <div className="flex-1">
                  <div className="service-card">
                    <div className="aspect-video bg-gradient-to-br from-primary/10 to-secondary/10 rounded-xl flex items-center justify-center overflow-hidden">
                      {service.image_url ? (
                        <img
